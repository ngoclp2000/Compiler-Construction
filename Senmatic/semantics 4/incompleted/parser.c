...
Type* compileLValue(void) {
  ...
  if (var->kind == OBJ_VARIABLE)
    varType = compileIndexes(var->typeAttrs->actualType);
...
void compileAssignSt(void) {
  ...
  Type *LvarType;
  Type *RvarType;
  LvarType =  compileLValue();
  eat(SB_ASSIGN);
  RvarType = compileExpression();
  checkTypeEquality(LvarType,RvarType);
}
...
void compileForSt(void) {
  ...
  checkTypeObj = checkDeclaredVariable(currentToken->string);
  checkBasicType(checkTypeObj->varAttrs->type);
  ...
  checkTypeEquality(checkTypeObj->varAttrs->type,exp1Type);
...
void compileCondition(void) {
  ...
  Type *LvarType =  compileExpression();
  checkBasicType(LvarType);
  switch (lookAhead->tokenType) {
  case SB_EQ:
    eat(SB_EQ);
    break;
  ...
  Type* RvarType =  compileExpression();
  checkBasicType(RvarType);
  checkTypeEquality(LvarType,RvarType);
}
...
Type* compileIndexes(Type* arrayType) {
  ...
  return arrayType; // when come to the last dimension
}
