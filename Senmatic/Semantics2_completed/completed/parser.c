...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      ...
      //printf("%d\n",constValue);
      constObj->constAttrs->value = duplicateConstantValue(constValue); 
      eat(SB_SEMICOLON);
...
void compileFuncDecl(void) {
  ...
  eat(SB_SEMICOLON);
  funcObject->funcAttrs->returnType = returnType;
  compileBlock();
  eat(SB_SEMICOLON);
  exitBlock();
}
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    Object *constObject =  lookupObject(currentToken->string);
    if(constObject == NULL) error(ERR_UNDECLARED_CONSTANT, lookAhead->lineNo, lookAhead->colNo);
    else if(constObject->kind != OBJ_CONSTANT) error(ERR_INVALID_CONSTANT, lookAhead->lineNo, lookAhead->colNo);
    constValue = duplicateConstantValue(constObject->constAttrs->value);
    break;
  case TK_CHAR:
    eat(TK_CHAR);
...
Type* compileType(void) {
  ...
  case KW_ARRAY:
    ...
    eat(SB_LSEL);
    eat(TK_NUMBER);
    int sizeArray = currentToken->value;
    eat(SB_RSEL);
    eat(KW_OF);
    Type* elementsType =  compileType();
    type = makeArrayType(sizeArray,elementsType);
    break;
  case TK_IDENT:
    eat(TK_IDENT);
    Object *typeObj = lookupObject(currentToken->string);
    if ( typeObj == NULL) error(ERR_UNDECLARED_TYPE, lookAhead->lineNo, lookAhead->colNo);
    else if(typeObj->kind != OBJ_TYPE) error(ERR_INVALID_TYPE, lookAhead->lineNo, lookAhead->colNo);
